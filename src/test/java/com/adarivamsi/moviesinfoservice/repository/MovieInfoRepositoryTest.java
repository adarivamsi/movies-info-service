...
class MovieInfoRepositoryTest {
    ...
    @Test
    void saveMovieInfo(){
        var movieInfo = new MovieInfo(null, "The Dark Knight New",
                2008, List.of("Christian Bale", "HeathLedger"), LocalDate.parse("2008-07-18"));
        var movieInfoMono = movieInfoRepository.save(movieInfo).log();
        StepVerifier.create(movieInfoMono)
                .assertNext(movieInfoObj -> {
                    assertNotNull(movieInfoObj.getMovieInfoId());
                    assertEquals("The Dark Knight New", movieInfoObj.getName());
                })
                .verifyComplete();
    }
    ...
    void updateMovieInfo(){
        ...
        StepVerifier.create(movieInfoMono)
                .assertNext(movieInfoObj -> {
                    assertEquals(2021, movieInfoObj.getYear());
                })
                .verifyComplete();
    }
